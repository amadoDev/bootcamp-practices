...
-- Creamos todas las tablas con sus claves primarias
...
alter table flota.vehiculo add constraint pk_vehiculo_modelo foreign key (id_modelo) references flota.modelo(id);
alter table flota.revision add constraint pk_revision_vehiculo foreign key (id_vehiculo) references flota.vehiculo(id);
...
alter table flota.poliza add constraint pk_poliza_vehiculo foreign key (id_vehiculo) references flota.vehiculo(id);
alter table flota.poliza add constraint pk_poliza_aseguradora foreign key (id_aseguradora) references flota.aseguradora(id);



-- Importamos los datos a una tabla auxiliar fuera del modelo llamada datos_flota:

CREATE TABLE flota.datos_flota (
...
INSERT INTO flota.datos_flota (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- Insertamos los datos en el esquema flota


insert into flota.grupo (nombre)
select grupo
from flota.datos_flota
group by grupo;
...
insert into flota.moneda (nombre, factor_euro)
select moneda, 1 as factor_euro
from flota.datos_flota
group by moneda;

insert into flota.marca (nombre, id_grupo)
select df.marca, g.id
from flota.datos_flota df
inner join flota.grupo g on g.nombre = df.grupo
group by g.id, df.marca
order by df.marca;

insert into flota.modelo (nombre, id_marca)
select df.modelo, m.id
from flota.datos_flota df
inner join flota.marca m on m.nombre = df.marca
group by m.id, df.modelo
order by df.modelo;

insert into flota.vehiculo (matricula, id_modelo, fecha_compra, color, kms_totales)
select df.matricula, m.id, df.fecha_compra, df.color, df.kms_totales
from flota.datos_flota df
inner join flota.modelo m on m.nombre = df.modelo
group by df.matricula, m.id, df.fecha_compra, df.color, df.kms_totales
order by df.kms_totales desc;

insert into flota.revision (id_vehiculo, kms_revision, fecha_revision, importe_revision, id_moneda)
select v.id, df.kms_revision, df.fecha_revision, df.importe_revision, m.id
from flota.datos_flota df
inner join flota.vehiculo v on v.matricula = df.matricula
inner join flota.moneda m on m.nombre = df.moneda
group by v.id, df.kms_revision, df.fecha_revision, df.importe_revision, m.id
order by df.fecha_revision;

insert into flota.poliza (numero, id_vehiculo, id_aseguradora, fecha_alta_seguro)
select df.n_poliza, v.id, a.id, df.fecha_alta_seguro
from flota.datos_flota df
inner join flota.vehiculo v on v.matricula = df.matricula 
inner join flota.aseguradora a on a.nombre = df.aseguradora
group by df.n_poliza, v.id, a.id, df.fecha_alta_seguro
order by df.n_poliza;


-- Eliminamos la tabla datos_flota

drop table flota.datos_flota;


-- Seleccionamos los siguientes datos de los coches activos:
...
select v.matricula, mo.nombre nombre_modelo, ma.nombre marca, g.nombre grupo, v.color, v.fecha_compra, v.kms_totales, p.numero poliza, a.nombre aseguradora
from flota.vehiculo v
inner join flota.modelo mo on mo.id = v.id_modelo
inner join flota.marca ma on ma.id = mo.id_marca
inner join flota.grupo g on g.id = ma.id_grupo
inner join flota.poliza p on p.id_vehiculo = v.id
